...
export function Login({ onLogin }) {
    ...
    const handleLogin = async (e) => {
        ...
        /* fetch('http://localhost:8080/api/login', {
            ...
            .then(data => {
                if (data.userId) {
                    localStorage.setItem('userToken', token_nhan_duoc);
        ...
        try {
            const response = await fetch('http://localhost:5000/api/auth/login', { // Kiểm tra lại route login của bạn
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email, password })
            });
            ...
            if (data.success) {
                // ĐÂY CHÍNH LÀ NƠI ĐẶT DÒNG CODE ĐÓ
                localStorage.setItem('userToken', data.token);
                console.log("Token nhận được:", data.token);

                // Thông báo cho component App rằng đã đăng nhập thành công
                onLogin();
        ...
        /* if (email === 'abc' && password === '123') {
            localStorage.setItem('userToken', token_nhan_duoc);
            onLogin(); // Gọi để biết là đã có người đăng nhập
            navigate('/');
        } */
    };
